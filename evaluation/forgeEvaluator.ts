...
export function evaluateExecution(
  ...
  if (isTokenUsageHigh(result.tokensUsed, budgetConfig.perTaskTokenLimit)) {
    partialReasons.push(`high_token_usage: ${result.tokensUsed}/${budgetConfig.perTaskTokenLimit}`);
  }

  if (isExecutionSlow(result.executionTimeMs)) {
    partialReasons.push(`slow_execution: ${result.executionTimeMs}ms`);
  }

  if (partialReasons.length > 0) {
    return { grade: "PARTIAL", reasons: partialReasons };
  }

  return { grade: "SUCCESS", reasons: [] };
}
