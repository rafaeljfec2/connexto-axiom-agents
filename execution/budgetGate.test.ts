...
describe('checkBudget', () => {
  ...
  it('não deve permitir orçamento se o orçamento mensal estiver esgotado', () => {
    ...
    expect(checkBudget(mockDb, 'agent1')).toEqual({ allowed: false, reason: 'Orcamento mensal esgotado (kill switch ativo)' });
  });
  ...
  it('não deve permitir orçamento se o limite mensal de tokens for excedido', () => {
    ...
    expect(checkBudget(mockDb, 'agent1')).toEqual({ allowed: false, reason: 'Limite mensal de tokens excedido (500001/500000)' });
  });
  ...
  it('não deve permitir orçamento se o limite mensal do agente for excedido', () => {
    ...
    expect(checkBudget(mockDb, 'agent1')).toEqual({ allowed: false, reason: 'Limite mensal do agente excedido (500001/500000)' });
  });
  ...
  it('não deve permitir orçamento se o limite diário de tarefas for excedido', () => {
    ...
    expect(checkBudget(mockDb, 'agent1')).toEqual({ allowed: false, reason: 'Limite diario de tasks excedido (11/10)' });
  });
});