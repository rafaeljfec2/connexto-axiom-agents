...
import type { FileEdit, FileChange } from "../project/projectSecurity.js";
...
function processJsonChar(ch: string, state: JsonScanState): boolean {
  if (state.escapeNext) {
...
export function extractFirstBalancedJson(text: string): string | null {
  ...
  const state: JsonScanState = { depth: 0, inString: false, escapeNext: false };

  for (let i = startIdx; i < text.length; i++) {
    if (processJsonChar(text[i], state)) {
