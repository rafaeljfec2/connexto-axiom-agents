...
function validateCodeOutputFields(raw: Record<string, unknown>): string | null {
  if (typeof raw.description !== "string" || raw.description.length === 0) {
    return "Missing or invalid description in code output";
  }
  if (typeof raw.risk !== "number" || raw.risk < 1 || raw.risk > 5) {
    return "Invalid risk value in code output";
  }
  if (!Array.isArray(raw.files) || raw.files.length === 0) {
    return "Missing or empty files array in code output";
  }
  return null;
}

function parseSingleFile(file: Record<string, unknown>): FileChange | null {
  if (typeof file.path !== "string" || file.path.length === 0) return null;
  if (file.action !== "create" && file.action !== "modify") return null;
...
function parseCodeOutput(text: string): ForgeCodeOutput | null {
  try {
    ...
    const validationError = validateCodeOutputFields(raw);
    if (validationError) {
    ...
    for (const file of raw.files as ReadonlyArray<Record<string, unknown>>) {
      const parsed = parseSingleFile(file);
      if (!parsed) {
        logger.error({ path: file.path }, "Invalid file entry in code output");
        return null;
      }
      files.push(parsed);
    }
