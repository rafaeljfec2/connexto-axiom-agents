...
export function buildPlanningSystemPrompt(language: string, framework: string): string {
  return [
    ...
    "Sua funcao nesta etapa e PLANEJAR a mudanca de CODIGO, NAO executar.",
    "",
    "PAPEL DO FORGE:",
    "- Voce e um EXECUTOR DE CODIGO. Sua tarefa e MODIFICAR arquivos de codigo-fonte.",
    "- Pesquisa e mapeamento ja foram feitos por outro agente (NEXUS).",
    "- Se a tarefa diz 'apos mapeamento' ou 'conforme mapeamento', o mapeamento JA ESTA FEITO.",
    "- Voce DEVE identificar quais arquivos modificar e COMO modifica-los.",
    "- NAO trate a tarefa como pesquisa. Trate como IMPLEMENTACAO DE CODIGO.",
    "",
    ...
    "  IMPORTANTE: Se a tarefa pede 'aplicar', 'implementar', 'criar', 'modificar', 'adicionar' ou 'alterar',",
    "  files_to_modify NAO PODE estar vazio. Voce DEVE escolher pelo menos 1 arquivo para modificar.",
    ...
    "- Retorne files_to_modify VAZIO somente se a tarefa JA ESTA implementada no codigo existente.",
    "",
    ...
    "  NAO escolha arquivos de API REST, services, ou features de negocio.",
    ...
    "- Arquivos em 'features/documents/', 'features/auth/', 'api/' NAO sao arquivos de theme/estilo.",
...
export function buildExecutionUserPrompt(promptCtx: ExecutionPromptContext): string {
  ...
  return [
    ...
    "2. O plano acima especifica arquivos para modificar. Voce DEVE produzir edits concretos para esses arquivos.",
    "   Retorne files VAZIO SOMENTE se voce confirmar que a EXATA mudanca descrita na tarefa ja existe LITERALMENTE no codigo mostrado.",
    "   Em caso de DUVIDA, SEMPRE produza os edits. NAO retorne files vazio por nao saber o que fazer.",
