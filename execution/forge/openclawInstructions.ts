...
export async function buildOpenClawInstructions(ctx: InstructionsContext): Promise<string> {
  ...
  const sections: string[] = [
    ...
    buildFrontendRules(ctx.language, ctx.framework),
    buildTestingRules(),
...
function buildToolUsageRules(baselineBuildFailed: boolean): string {
  ...
  lines.push(
    ...
    "7. When done, provide a structured delivery summary.",
    "",
...
function buildFrontendRules(language: string, framework: string): string {
  const frontendIndicators = ["react", "next", "vue", "angular", "svelte", "solid", "astro", "remix"];
  const combined = `${language} ${framework}`.toLowerCase();
  if (!frontendIndicators.some((indicator) => combined.includes(indicator))) return "";
...
function buildDependencyRules(): string {
  return [
    "# Dependency Rules",
    "",
