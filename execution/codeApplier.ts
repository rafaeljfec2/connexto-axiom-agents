...
const PROTECTED_FILES: readonly string[] = ["state/schema.sql"];

const ALLOWED_EXTENSIONS: readonly string[] = [".ts", ".js", ".json", ".sql", ".md"];
...
export function validateFilePaths(files: readonly FileChange[]): {
  ...
  for (const file of files) {
    ...
    const isProtected = PROTECTED_FILES.some((p) => normalized === p);
    if (isProtected) {
